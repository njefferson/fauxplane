 ...
 * text you can paste.
 *
 * WHY THIS EXISTS. Every defect this app has had was found on a real device
 * photographing a phone and me reading pixels: a residual figure, a flag, a
...
export function buildReport({ snapshot, fusion, traffic, route = null, selfTest = null, visibility = null, metar, bootAt, precisePosition = false, env = {}, mount = null, mountApplies = null, raw = {}, sensors = null, now = null }) {
  ...
  // `coasting -9ms`, `-21ms`, `-34ms` in every report received: the last
  ...
  // into the surface that a real device can run. This costs no extra request —
  ...
  // from a real device, on the first real follow.
  //
  ...
  if (rp) {
     ...
     *
     * the 1.23.1 report has this block reading `callsign N460DF` while the
